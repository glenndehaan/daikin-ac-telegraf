...
const run = async () => {
    ...
    for(let item = 0; item < devices.length; item++) {
        ...
        if(typeof result !== "undefined") {
            ...
            console.log(`daikinAc,ip=${device},name=${data.basic.name},macAddress=${data.basic.macAddress} power=${data.status.power},targetTemperature=${data.status.targetTemperature},fanRate=${data.status.fanRate},indoorTemperature=${data.sensor.indoorTemperature},outdoorTemperature=${data.sensor.outdoorTemperature},todayRuntime=${data.power.todayRuntime}`);
        }
    }
};
